...
stati(struct inode *ip, struct stat *st)
{
	...
	if(ip->type == 4) {
		while(i < 12 && ip->addrs[i] != 0)
		{
			st->checksum ^= (char) ((ip->addrs[i] & 0xFF000000)>>24);
			i++; 
		}

		if(i==12 && ip->addrs[i] != 0)
		{
			i = 0;
			// Load indirect block, allocating if necessary.
			ind_addr = ip->addrs[NDIRECT];
			bp_indirect = bread(ip->dev, ind_addr);
			a = (uint*)bp_indirect->data;
			while (i < (512/4) && a[i] != 0)
			{
				st->checksum ^= (char) ((a[i] & 0xFF000000)>>24);
				i++; 
			}		
		}
	}
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
	...
	checksum = (char)((addr & 0xFF000000)>>24);
	addr = (addr & 0x00FFFFFF);
	}
    //bp = bread(ip->dev, bmap(ip, off/BSIZE));
    bp = bread(ip->dev, addr);
    if(ip->type == T_CHECKED) {
	checksum2 = calcCheckSum(bp);
...
{
	uint tot, m, ind_addr, newaddr, addr, bn, *a;
	struct buf *bp;
	struct buf *bp_indirect;
	char checksum;
	...
	for(tot=0; tot<n; tot+=m, off+=m, src+=m){
		...
		if(ip->type == T_CHECKED) {
			bn = (off/BSIZE);
			checksum = calcCheckSum(bp);
			newaddr = addr | (checksum << 24);
			if(bn < NDIRECT){
				ip->addrs[bn] = newaddr;
			} else {
				bn -= NDIRECT;
