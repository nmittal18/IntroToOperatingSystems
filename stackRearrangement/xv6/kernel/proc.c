...
//  cprintf("NEHA PROC : before copyuvm\n");

  // Copy process state from p.
//  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz, proc->user_stack)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
...
//  cprintf("NEHA FORK in PROC.C stack pointer user_stack val %x\n", proc->user_stack);

  np->sz = proc->sz;
  np->user_stack = proc->user_stack;  //ADDED
  np->parent = proc;
  *np->tf = *proc->tf;
...
//  cprintf("NEHA PROC : process name before return %s\n", np->name);
//  cprintf("NEHA PROC : stack pointer before return %x\n", np->tf->esp);
//  cprintf("NEHA PROC : user_stack before return %x\n", np->user_stack);
  return pid;
}
